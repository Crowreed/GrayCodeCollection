...
vector<int> listepower2(int n) {
    //Retourne un vecteur de toutes les puissances de 2 sur n bits
    vector<int> res;
    for (int i=0; i<pow(2,n); i++) {
        if (pw2(i)) {
            res.push_back(i);
...
vector<bool> listepower2Bool(int n) {
    //Retourne un vecteur pour savoir si l'entier à la position n est une puissance de 2, cosntructeur de p2bool
    vector<bool> res;
    for (int i=0; i<pow(2,n); i++) {
        if (pw2(i)){
            res.push_back(true);
...
void BGC(int x) {
    ...
    if (p2Bool[x]){
        //Si l'entier courant est une puissance de 2
        bool flag=false;
        for (int i : p2){
            //Test pour savoir si il reste des puissances de 2 à explorer
            if (avail[i]){
                flag=true;
                break;
            }
        ...
        if (!flag) {
            //Si il ne reste plus de puissance de 2 on arrête.
            return;
        }
    }
