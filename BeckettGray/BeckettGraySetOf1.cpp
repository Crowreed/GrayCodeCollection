...
vector<int> listeVoisinV; //Nombre d'arcs entre ensembles avec le même nombre de 1
...
void BGC(int x) {
    ...
    if (cmpt==1 && (bi[xn][bit]==1 || bit==old[0])){
        ...
        if (V[x]>V[xn]){
            //On met à jour le nombre d'arcs disponible entre ensembles de même nombre de 1
            listeVoisinV[V[xn]]--;
        }
        else {
            listeVoisinV[V[x]]--;
    ...
    //Si on a pas de sommet isolé
        if (!(V[x]==0 or V[x]==n) && listeVoisinV[V[x]-1]==1 && listeVoisinV[V[x]]>0){
            //Si il n'y a plus qu'un seul chemin entre l'ensembel avec un 1 en moins et plus de 0 dans l'ensemble avec un 1 en plus
            for (int i=0; i<n; i++) {
                bit = n-i-1;
                if (bi[x][bit]==0){
                    //On parcours les sommets avec un 1 en plus
                    xn = flip[x][i];
